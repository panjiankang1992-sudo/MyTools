...
grep -Fq '.parallelGesture(PanGesture({ fingers: 1, direction: PanDirection.Horizontal, distance: 12 })' "$PAGE"
...
grep -Fq ".accessibilityText('下一页').onClick(() => this.ChangeReaderPageFromFooter(1))" "$PAGE"
grep -Fq 'private PrepareReaderContinuousWindowAtChapter(index: number): void' "$PAGE"
grep -Fq 'if (this.currentBook !== undefined) this.currentBook.progress = 100;' "$PAGE"
