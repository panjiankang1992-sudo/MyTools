...
grep -Fq 'this.ScheduleShelfBookLocalCache(stored, this.detailCatalog);' "$PAGE"
grep -Fq 'private async CacheShelfBookWindow(' "$PAGE"
grep -Fq 'startIndex + this.readerChapterPrefetchCount' "$PAGE"
grep -Fq 'private ShelfBookCacheCurrent(' "$PAGE"
...
grep -Fq 'MAX_CHAPTER_FILES_PER_BOOK: number = 10000' "$CACHE"
