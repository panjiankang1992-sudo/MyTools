...
public class LegacyIdentityExportController {
 ...
 /** 分页导出身份最小字段。 @param authorization 授权 @param afterId 游标 @param limit 大小 @return 页面 */
 @GetMapping public ExportPage export(@RequestHeader("Authorization") String authorization,@RequestParam(defaultValue="0") long afterId,@RequestParam(defaultValue="100") int limit){authorize(authorization);if(afterId<0||limit<1||limit>500)throw new IllegalArgumentException("identity migration page is invalid");
  List<ExportUser> users=mapper.selectIdentityMigrationBatch(afterId,limit).stream().map(user->{String role=user.getRole()==null||!user.getRole().matches("^[A-Z][A-Z0-9_]{0,63}$")?"USER":user.getRole();return new ExportUser(user.getId(),"mytools:"+user.getId(),user.getUsername(),user.getEmail(),user.getPassword(),user.getStatus(),0,List.of(role));}).toList();
  return new ExportPage(users,users.isEmpty()?afterId:users.getLast().id(),users.size()<limit);
 }
 ...
 /** 身份迁移页面。 */ public record ExportPage(List<ExportUser> users,long nextAfterId,boolean complete) { }
}
