...
/**
 * 注册邮件真实投递 Outbox 数据访问层。
 */
@Mapper
public interface RegistrationMailDeliveryOutboxMapper {

    /** 插入 Outbox。 @param record 记录 @return 影响行数 */
    int insert(RegistrationMailDeliveryOutbox record);

    /** 查询可领取记录。 @param now 当前时间 @param limit 上限 @return 记录列表 */
    List<RegistrationMailDeliveryOutbox> findReady(@Param("now") LocalDateTime now, @Param("limit") int limit);
    ...
    /** 记录失败。 @param verificationId 验证码标识 @param claimedUntil 领取令牌 @param status 状态 @param attemptCount 尝试次数 @param availableAt 下次时间 @param errorCode 错误码 @param now 当前时间 @return 影响行数 */
