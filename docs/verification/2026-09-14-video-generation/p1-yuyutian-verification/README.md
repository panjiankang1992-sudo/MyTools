...
虚拟机实测：`02-tool-page-as-yuyutian.jpeg` 之后的 `imagework-entry.jpeg` 能看到新入口与素材门禁；
`app/build/acceptance/video-generation/imagework-sheet-final.jpeg` 与布局 dump 显示面板能打开、
列出该账号的作品并有「用作素材」按钮。面板内的选择点击在模拟器上没走完（输入注入时好时坏），
所以"生图 → 当首帧 → 出片"这条链改用同一批接口在 yuyutian 会话下验完：
...
| 本轮 Krea 生成的青瓷茶壶图 | f0 (26,109,186)、f24 (23,115,207) 明显偏蓝 | f0 0.36 |
...
## 八、这次验证的边界（说清楚，别当成已验证）

1. **App 控件级的"选作品 → 用作素材 → 提交"没在模拟器里走完**（输入注入时好时坏，见上）；
   但面板能打开、能列出作品（有 dump 与截图），背后的调用序列已在同一账号下逐条验过。
