...
1. **App 虚拟机走查已做，但没提交真实任务**：2026-09-19 在本地 HarmonyOS 模拟器上走查了入口、
...
2. **`video_generate/1.0.0` 不写 `outputs/<jobId>/result.json`**：服务已能自给自足（DEF-15），
   但工具的 `result_json.resourcePeak` 仍为空；如需在工具侧展示显存峰值，需要发布
   `video_generate/1.0.1`（把执行器回报的结果同时落一份到受管输出目录）并更新任务定义版本。
3. 其余模式（文生、两图参考、首尾帧、结构重绘、局部修改）保持关闭，直到各自在维护窗口里
