...
class Cursor:
 ...
 def execute(self,sql,args=None):
  if "information_schema.tables"in sql:self.rows=[{"value":1}]
  elif "MAX(id)"in sql:self.rows=[{"value":1}]
...
def test_treats_known_absent_feedback_table_as_empty_source():
 class MissingCursor(Cursor):
  def execute(self,sql,args=None):
   if "information_schema.tables"in sql:self.rows=[{"value":0}]
   elif "FROM t_feedback"in sql:raise AssertionError("absent table must not be queried")
   else:self.rows=[]
 class MissingConnection(Connection):
  def cursor(self):return MissingCursor(self)
 connection=MissingConnection();client=Client();result=MODULE.execute(connection,client,"feedback-empty",True)
 assert result["sourceHighWater"]==0 and result["exported"]==0
 assert result["accepted"]==0 and result["rejected"]==0 and connection.rolled
