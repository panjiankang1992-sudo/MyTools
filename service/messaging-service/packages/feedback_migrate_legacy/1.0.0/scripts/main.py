...
def execute(connection,client:Client,key:str,dry:bool)->dict:
 ...
 with connection.cursor()as cursor:cursor.execute("SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ");cursor.execute("START TRANSACTION WITH CONSISTENT SNAPSHOT, READ ONLY");cursor.execute("SELECT COALESCE(MAX(id),0) value FROM t_feedback");high=int(cursor.fetchone()["value"])
 ...
 while after<high:
  ...
  for value in values:
   encoded=canonical(value);value_digest=hashlib.sha256(encoded).hexdigest()
