...
def test_accepts_production_sized_tag_sets_with_a_bounded_limit():
    tags = [{"tag_name": f"tag-{index}", "confidence": 0.5} for index in range(324)]
    assert len(MODULE.normalize_tags(tags)) == 324
    with pytest.raises(ValueError, match="TAG_COUNT_INVALID"):
        MODULE.normalize_tags(tags + [
            {"tag_name": f"extra-{index}", "confidence": 0.5} for index in range(189)
        ])
