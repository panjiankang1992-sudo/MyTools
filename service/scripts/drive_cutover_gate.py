#!/usr/bin/env python3
"""Validate frozen Drive account migration evidence without changing runtime state."""
...
def evaluate(dry_run: dict, applied: dict, replay: dict, target: dict) -> dict:
    """Evaluate source freezing, idempotency, and target collection equality."""
...
def read_high_water(report: dict, errors: set[str]) -> dict[str, int] | None:
    """Read the exact two-source frozen cursor map."""
    ...
    if not isinstance(value, dict) or set(value) != set(SOURCES):
        ...
        return None
    result = {source: value.get(source) for source in SOURCES}
...
def main() -> None:
    """Run the read-only Drive migration cutover gate."""
