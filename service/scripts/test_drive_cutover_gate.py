"""Offline Drive account migration cutover gate tests."""

import copy
import unittest

from service.scripts.drive_cutover_gate import evaluate


def report(dry_run: bool, accepted: int, skipped: int) -> dict:
    """Build one valid migration report fixture."""

    return {"migrationKey": "drive-accounts-2026", "dryRun": dry_run,
            "exported": 4, "accepted": accepted, "skipped": skipped, "rejected": 0,
            "digestSha256": "a" * 64, "sourceHighWater": {"DRIVE": 9, "WEBDAV": 3}}


class DriveCutoverGateTest(unittest.TestCase):
    """Cover safe and unsafe Drive migration evidence combinations."""

    def setUp(self):
        self.dry_run = report(True, 4, 0)
        self.applied = report(False, 4, 0)
        self.replay = report(False, 0, 4)
        self.target = {"migrationKey": "drive-accounts-2026", "itemCount": 4,
                       "digestSha256": "a" * 64}
    ...
    def test_rejects_source_high_water_drift(self):
        ...
        replay["sourceHighWater"]["DRIVE"] = 10
