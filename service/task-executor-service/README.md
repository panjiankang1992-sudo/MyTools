...
当前已实现节点注册与心跳、任务领取、执行租约续期、取消感知、脚本入口安全解析、任务上下文/结果文件、普通及场景步骤顺序执行、步骤重试和结果回传。Python SDK 统一提供任务控制、Storage Gateway 访问、有边界的电子书文本和归档读取能力，以及隔离的 Reader Runtime 客户端，避免各脚本重复实现鉴权和安全校验。脚本包发布目录结构：
