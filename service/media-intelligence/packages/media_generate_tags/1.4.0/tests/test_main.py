...
class MediaGenerateTagsTest(unittest.TestCase):
    ...
    def test_model_selection_respects_explicit_task_and_environment(self):
        """默认使用八十亿参数模型，同时保留显式任务参数的优先级。"""
        with tempfile.TemporaryDirectory() as directory:
            source = Path(directory) / "sample.txt"
            source.write_text("A forest")
            parameters = {"sourcePath": str(source), "filename": source.name, "mimeType": "text/plain"}
            with patch.dict(os.environ, {}, clear=True):
                _, payload = MODULE.build_request(parameters)
                self.assertEqual("huihui_ai/qwen3-vl-abliterated:8b", payload["model"])
            with patch.dict(os.environ, {"TAGGING_MODEL": "custom:8b"}):
                _, payload = MODULE.build_request(parameters)
                self.assertEqual("custom:8b", payload["model"])
                _, payload = MODULE.build_request({**parameters, "model": "explicit:8b"})
                self.assertEqual("explicit:8b", payload["model"])
