...
class AssetRegisterContentTest(unittest.TestCase):
    ...
    def test_detected_mime_overrides_generic_input(self):
        client = Client()
        MODULE.execute({"parameters": {"downloadRequestId": "request", "assetMimeType": "application/octet-stream"},
                        "stepOutputs": {"publish_asset": {"storageUri": "storage://managed/photo.jpg",
                            "contentSha256": "a" * 64, "sizeBytes": 20, "mimeType": "image/jpeg"}}}, client)
        self.assertEqual("image/jpeg", client.payload["mimeType"])
