...
# 端到端用子进程跑 worker，但期望值（例如控制强度默认值）仍从同一份实现读取，避免测试写死数字。
sys.path.insert(0, str(SCRIPTS))
import vace_control as vc  # noqa: E402
...
class EndToEndTest(unittest.TestCase):
    ...
    def test_first_frame_runs_end_to_end(self):
        ...
        # 控制强度与包内默认一致（可被 VIDEO_FILL_BLEND 覆盖，但默认必须如实记录在结果里）。
        self.assertEqual(result['fillBlend'], vc.DEFAULT_FILL_BLEND)
        self.assertGreater(result['inferenceMillis'], 0)
        self.assertGreater(result['resourcePeak']['gpuPeakMiB'], 0)
        self.assertEqual(result['colorMatch'], 1.0)
        self.assertEqual(len(result['colorOffsetLast']), 3)
