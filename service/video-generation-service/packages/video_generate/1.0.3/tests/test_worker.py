...
class ColorMatchTest(unittest.TestCase):
    """出片颜色校正：消色偏但不动结构与补边，强度可关。"""

    def frame(self, base, cast=(0, 0, 0)):
        frame = np.full((480, 832, 3), base, dtype=np.uint8)
        frame[:, :, 0] = np.clip(frame[:, :, 0].astype(int) + cast[0], 0, 255)
        frame[:, :, 1] = np.clip(frame[:, :, 1].astype(int) + cast[1], 0, 255)
        frame[:, :, 2] = np.clip(frame[:, :, 2].astype(int) + cast[2], 0, 255)
        return frame

    def content_mean(self, frame, rects=(176, 176, 0, 0)):
        rows, columns = vc.content_region(rects, 832, 480)
        return frame[rows, columns].reshape(-1, 3).astype(np.float32).mean(axis=0)

    def test_cast_is_removed_while_bars_stay_neutral(self):
        rects = (176, 176, 0, 0)
        frames = np.stack([self.frame(120), self.frame(120, (30, 0, -60))])
        # 两侧补边保持中性灰，校正后应仍是灰（整帧平移会带动它，但补边还原会盖回来）。
        for index in range(2):
            frames[index][:, :176] = vc.PAD_COLOR_RGB
            frames[index][:, 832 - 176:] = vc.PAD_COLOR_RGB
        offsets = vc.match_reference(frames, rects, 1.0)
        self.assertAlmostEqual(float(self.content_mean(frames[0])[0]), float(self.content_mean(frames[1])[0]), delta=1.0)
        self.assertAlmostEqual(float(self.content_mean(frames[1])[2]), float(self.content_mean(frames[0])[2]), delta=1.0)
        self.assertNotEqual(offsets, [0, 0, 0])

    def test_structural_difference_survives(self):
        rects = (176, 176, 0, 0)
        left = np.full((480, 832, 3), 100, dtype=np.uint8)
        right = left.copy()
        right[:, 416:] = 180           # 结构差异，但整体均值可以对齐
        frames = np.stack([left, right])
        vc.match_reference(frames, rects, 1.0)
        # 平移无法抹掉结构差：两帧在对应区域仍明显不同。
        self.assertGreaterEqual(abs(int(frames[1][240, 600, 0]) - int(frames[0][240, 600, 0])), 40)

    def test_zero_strength_is_noop(self):
        rects = (176, 176, 0, 0)
        frames = np.stack([self.frame(120), self.frame(120, (30, 0, -60))])
        before = frames.copy()
        self.assertEqual(vc.match_reference(frames, rects, 0.0), [0, 0, 0])
        self.assertTrue(np.array_equal(before, frames))

    def test_environment_is_validated(self):
        saved = os.environ.pop('VIDEO_COLOR_MATCH', None)
        try:
            self.assertEqual(worker.color_match_strength(), 1.0)
            os.environ['VIDEO_COLOR_MATCH'] = '0'
            self.assertEqual(worker.color_match_strength(), 0.0)
            os.environ['VIDEO_COLOR_MATCH'] = '0.5'
            self.assertAlmostEqual(worker.color_match_strength(), 0.5)
            for value in ('-0.1', '1.5', 'abc'):
                os.environ['VIDEO_COLOR_MATCH'] = value
                with self.assertRaises(worker.Rejected) as caught:
                    worker.color_match_strength()
                self.assertEqual(caught.exception.code, 'VIDEO_COLOR_MATCH_INVALID')
        finally:
            os.environ.pop('VIDEO_COLOR_MATCH', None)
            if saved is not None:
                os.environ['VIDEO_COLOR_MATCH'] = saved
