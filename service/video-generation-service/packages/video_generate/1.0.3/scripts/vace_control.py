...
def decode_frames(paths, width, height, work_dir):
    """一次解码全部生成帧为 (T, H, W, 3) uint8，用于出片前的颜色校正。"""
    listing = Path(work_dir) / 'decode-frames.txt'
    # concat 列表必须带 duration：静态图片没有时长时 demuxer 只吐出几帧（实测 49 张只出 4 帧），
    # 而 run_ffmpeg 的整帧字节校验会立刻发现这种"少帧"。
    listing.write_text(''.join(f"file '{path}'\nduration 0.04\n" for path in paths))
    raw = run_ffmpeg(['ffmpeg', '-v', 'error', '-f', 'concat', '-safe', '0', '-i', str(listing),
                      '-frames:v', str(len(paths)), '-f', 'rawvideo', '-pix_fmt', 'rgb24', '-'],
                     expected_bytes=len(paths) * height * width * 3)
    return np.frombuffer(raw, dtype=np.uint8).reshape(len(paths), height, width, 3).copy()


def content_region(rects, width, height, margin=8):
    """内容区（排除补边，再留一点边距），用于统计颜色而不被恒定的补边带偏。"""
    left, right, top, bottom = rects
    x0 = min(width, left + margin)
    x1 = max(x0 + 1, width - right - margin)
    y0 = min(height, top + margin)
    y1 = max(y0 + 1, height - bottom - margin)
    return slice(y0, y1), slice(x0, x1)


def match_reference(frames, rects, strength=1.0):
    """把每帧的通道均值对齐到首帧的内容区，消除模型引入的整体色偏。

    只做**逐通道平移**（不改增益，因此不动对比度与结构）。整帧平移会把补边一起带动，所以平移后
    立刻按同一几何把补边盖回中性灰——这样封面（直接用首帧写 PNG）与成片都不依赖后续步骤。
    `strength` 为 0 时等于不做。返回末帧实际应用的偏移，作为证据。
    """
    if strength <= 0 or frames.shape[0] == 0:
        return [0, 0, 0]
    height, width = frames.shape[1:3]
    rows, columns = content_region(rects, width, height)
    reference = frames[0][rows, columns].reshape(-1, 3).astype(np.float32).mean(axis=0)
    offsets = [0, 0, 0]
    for index in range(frames.shape[0]):
        current = frames[index][rows, columns].reshape(-1, 3).astype(np.float32).mean(axis=0)
        delta = (reference - current) * strength
        if index == frames.shape[0] - 1:
            offsets = [round(float(value), 2) for value in delta]
        shifted = frames[index].astype(np.float32) + delta
        frames[index] = np.clip(shifted, 0, 255).astype(np.uint8)
        restore_pad(frames[index], rects)
    return offsets


def restore_pad(frame, rects):
    """按补边几何把两侧/上下盖回中性灰（numpy 版），与 drawbox 滤镜同源。"""
    left, right, top, bottom = rects
    height, width = frame.shape[:2]
    if left > 0:
        frame[:, :min(left, width)] = PAD_COLOR_RGB
    if right > 0:
        frame[:, max(0, width - right):] = PAD_COLOR_RGB
    if top > 0:
        frame[:min(top, height), :] = PAD_COLOR_RGB
    if bottom > 0:
        frame[max(0, height - bottom):, :] = PAD_COLOR_RGB


def write_png(frame, path):
    """把单帧写成 PNG（封面用），与成片共用同一份已校正的像素。"""
    height, width = frame.shape[:2]
    subprocess.run(['ffmpeg', '-v', 'error', '-y', '-f', 'rawvideo', '-pix_fmt', 'rgb24',
                    '-s', f'{width}x{height}', '-i', '-', '-frames:v', '1', str(path)],
                   input=frame.astype(np.uint8).tobytes(), check=True, timeout=120)
