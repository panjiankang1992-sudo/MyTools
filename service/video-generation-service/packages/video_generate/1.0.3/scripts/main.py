...
def fill_blend():
    """控制强度：默认取包内常量，允许运维用 VIDEO_FILL_BLEND 覆盖（客户端不能指定）。

    调参属于运维动作：值非法时直接失败，而不是静默回退成默认值——否则窗口里换了参数却没人发现。
    """
    raw = os.environ.get('VIDEO_FILL_BLEND', '')
    if not raw:
        return vc.DEFAULT_FILL_BLEND
    try:
        value = float(raw)
    except ValueError:
        raise Rejected('VIDEO_FILL_BLEND_INVALID') from None
    if not 0.0 < value < 1.0:
        raise Rejected('VIDEO_FILL_BLEND_INVALID')
    return value


def color_match_strength():
    """出片后的颜色校正强度：默认 1.0（把每帧内容区均值对齐首帧），0 表示关闭。

    白底素材在首帧模式下会把内容区漂成蓝色（实测末帧 R−B 变化 −109~−176），只调控制强度不足以解决
    （seed 932 在 α=0.50 下仍漂 115），因此增加这一步与模型无关的确定性校正。
    """
    raw = os.environ.get('VIDEO_COLOR_MATCH', '')
    if not raw:
        return 1.0
    try:
        value = float(raw)
    except ValueError:
        raise Rejected('VIDEO_COLOR_MATCH_INVALID') from None
    if not 0.0 <= value <= 1.0:
        raise Rejected('VIDEO_COLOR_MATCH_INVALID')
    return value
...
def prepare_control(mode, files, work, width, height, frames):
    ...
    if mode in ('FIRST_FRAME', 'FIRST_LAST_FRAMES'):
        ...
        blend = fill_blend()
        control, mask = vc.reference_frames(stacked, indices, 'blend', blend)
        manifest.update(referenceFill='blend', fillBlend=blend, preservedFrameIndices=indices)
...
def encode_frames(frames, target, work, restore=None):
    """把已校正的帧序列编码为 H.264/yuv420p 并加 faststart。

    `restore` 是把补边还原为中性灰的滤镜：模型会把补边区改成任意颜色，这里按控制帧量出的
    几何覆盖回去，保证成片两侧仍是约定的中性灰。帧通过 rawvideo 管道进入编码器，与颜色校正
    共用同一份像素，避免中间再落盘一轮图片。
    """
    ...
    if restore:
        args += ['-vf', restore]
    args += ['-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', str(target)]
    subprocess.run(args, input=frames.astype(np.uint8).tobytes(), check=True, timeout=900)
...
def generate(parameters):
    ...
    with GpuLease(), PromptCleanup(base) as cleanup:
        ...
        # 颜色校正必须在补边还原之前做：先整帧平移消色偏，再让 drawbox 把两侧盖回中性灰。
        strength = color_match_strength()
        frames = vc.decode_frames(paths, width, height, work)
        rects = (manifest['padRects']['left'], manifest['padRects']['right'],
                 manifest['padRects']['top'], manifest['padRects']['bottom'])
        color_offsets = vc.match_reference(frames, rects, strength)
        manifest['colorMatch'] = strength
        manifest['colorOffsetLast'] = color_offsets
        corrected = output / 'video.mp4'
        encode_frames(frames, corrected, work, restore)
        vc.write_png(frames[0], output / 'cover.png')
        for path in paths:
