...
def encode_video(paths, target, work):
    ...
    subprocess.run(['ffmpeg', '-v', 'error', '-y', '-f', 'concat', '-safe', '0', '-i', str(listing),
                    '-r', str(FPS), '-frames:v', str(FRAMES), '-c:v', 'libx264', '-pix_fmt', 'yuv420p',
                    '-movflags', '+faststart', str(target)], check=True, timeout=900)
...
def generate(parameters):
    ...
    with GpuLease(), PromptCleanup(base) as cleanup:
        ...
        encode_video(paths, output / 'video.mp4', work)
        atomic_write(output / 'cover.png', paths[0].read_bytes())
        for path in paths:
    ...
    for key in ('referenceFill', 'fillBlend', 'sourceQuality'):
        if key in manifest:
