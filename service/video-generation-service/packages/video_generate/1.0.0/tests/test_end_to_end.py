    ...
    def do_POST(self):
        ...
        if self.path == '/free':
            ...
            if self.server.fail_next:
                ...
                self.server.frames[prompt_id] = write_frames(self.server.output_dir,
...
def write_frames(root, prefix, count):
    ...
    for index in range(1, count + 1):
        ...
        subprocess.run(['ffmpeg', '-v', 'error', '-y', '-f', 'lavfi',
                        '-i', f'color=c=0x{index % 200 + 20:02x}8080:s=832x480', '-frames:v', '1', str(path)],
