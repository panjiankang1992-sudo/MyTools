...
def test_multiple_files_keep_distinct_stable_source_indices_on_recovery():
    items = [{"remoteFileId": "file-" + str(i), "relativePath": "book/" + str(i) + ".epub",
              "sizeBytes": 7, "storageProviderId": str(uuid4()), "storagePath": "ready/" + str(i)}
             for i in range(10)]
    operation = {"id": str(uuid4()), "phase": "READY", "items": items}
    parameters = {"downloadRequestId": str(uuid4()), "accountId": str(uuid4()),
                  "magnetUri": "magnet:?xt=urn:btih:" + "a" * 40}
    normal, recovery = FakeContext(parameters), FakeContext({**parameters, "recoveryAttempt": 2})
    MODULE.execute(normal, FakeClient([operation]))
    MODULE.execute(recovery, FakeClient([operation]))
    assert [child[1]["sourceIndex"] for child in normal.created] == list(range(10))
    assert [child[1] for child in normal.created] == [child[1] for child in recovery.created]

def test_recovery_skips_only_completed_matching_items():
    import pytest
    operation_id = str(uuid4())
    items = [{"remoteFileId": "f" + str(i), "relativePath": str(i) + ".gif", "sizeBytes": 7,
              "storageProviderId": str(uuid4()), "storagePath": "ready/" + str(i)} for i in range(3)]
    operation = {"id": operation_id, "phase": "READY", "items": items}
    parameters = {"downloadRequestId": str(uuid4()), "accountId": str(uuid4()),
                  "magnetUri": "magnet:?xt=urn:btih:" + "a" * 40, "recoveryAttempt": 3}
    completed = [{"itemId": f"pikpak:{operation_id}:1", "tagStatus": "TAGGED",
                  "sizeBytes": 7, "contentSha256": "a" * 64},
                 {"itemId": f"pikpak:{operation_id}:2", "tagStatus": "PENDING",
                  "sizeBytes": 7, "contentSha256": "b" * 64}]
    context = FakeContext(parameters)
    MODULE.execute(context, FakeClient([operation]), recovery_results=completed)
    assert [child[1]["sourceIndex"] for child in context.created] == [1, 2]
    invalid = [{**completed[0], "sizeBytes": 8}]
    with pytest.raises(RuntimeError, match="identity mismatch"):
        MODULE.execute(FakeContext(parameters), FakeClient([operation]), recovery_results=invalid)

def test_recovery_with_all_results_needs_no_children():
    operation_id = str(uuid4())
    item = {"remoteFileId": "f", "relativePath": "a.gif", "sizeBytes": 7,
            "storageProviderId": str(uuid4()), "storagePath": "ready/a.gif"}
    operation = {"id": operation_id, "phase": "READY", "items": [item]}
    context = FakeContext({"downloadRequestId": str(uuid4()), "accountId": str(uuid4()),
        "magnetUri": "magnet:?xt=urn:btih:" + "a" * 40, "recoveryAttempt": 3})
    result = MODULE.execute(context, FakeClient([operation]), recovery_results=[{
        "itemId": f"pikpak:{operation_id}:1", "tagStatus": "TAGGED", "sizeBytes": 7,
        "contentSha256": "a" * 64}])
    assert result["status"] == "READY" and result["childTaskIds"] == []
