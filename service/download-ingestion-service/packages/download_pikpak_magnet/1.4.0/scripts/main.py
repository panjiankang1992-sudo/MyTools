...
class ConnectorClient:
    ...
    def __init__(self, base_url: str, token: str, timeout: float = 195):
...
def execute(context: TaskContext, client: ConnectorClient, sleeper=time.sleep, recovery_results=None) -> dict:
    ...
    recovery = parameters.get("recoveryAttempt", 0)
    if not isinstance(recovery, int) or isinstance(recovery, bool) or not 0 <= recovery <= 10:
        raise ValueError("recoveryAttempt is outside the supported range")
    ...
    # 人工补跑只允许复用已完成的云端操作，绝不重新提交磁力或移动云端文件。
    if recovery and operation.get("phase") != "READY":
        raise RuntimeError("PikPak recovery requires an existing ready operation")
    ...
    children = []
    completed = {str(item["itemId"]): item for item in (recovery_results or [])} if recovery else {}
    ...
    for index, item in enumerate(safe_items, start=1):
        ...
        existing = completed.get(item_id)
        if existing is not None and existing.get("tagStatus") in {"TAGGED", "SKIPPED"}:
            # 补跑复用已持久化的完整结果，禁止重复下载及非确定性的二次打标签。
            if int(existing.get("sizeBytes", -1)) != item["sizeBytes"] or not re.fullmatch(
                    r"[a-f0-9]{64}", str(existing.get("contentSha256", ""))):
                raise RuntimeError("PikPak recovery result identity mismatch")
            continue
        # 每个文件使用稳定且唯一的来源序号，避免多文件入库命中唯一键冲突。
        child = context.create_child("download_remote_storage_object", {
            "downloadRequestId": request_id, "itemId": item_id, "sourceIndex": index - 1,
            ...
            "destinationRootName": str(parameters.get("destinationRootName") or "managed"),
...
def main() -> None:
    ...
    recovered = None
    if context.parameters.get("recoveryAttempt"):
        # 从所有者隔离的下载接口读取成功检查点，不让调用方伪造完成结果。
        request_id = str(UUID(str(context.parameters["downloadRequestId"])))
        owner_id = int(context.parameters.get("ownerId") or 0)
        request = Request(os.getenv("DOWNLOAD_INGESTION_URL", "http://127.0.0.1:23220").rstrip("/")
            + f"/internal/v1/download-requests/{request_id}/result-summary?ownerId={owner_id}",
            headers={"Authorization": "Bearer " + os.environ["DOWNLOAD_INTERNAL_TOKEN"]})
        with urlopen(request, timeout=30) as response:
            summary = json.loads(response.read(16 * 1024 * 1024))
        if str(summary.get("downloadRequestId")) != request_id or not isinstance(summary.get("items"), list):
            raise RuntimeError("PikPak recovery returned an invalid download summary")
        recovered = summary["items"]
    write_result(execute(context, client, recovery_results=recovered))
