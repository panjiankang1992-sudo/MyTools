...
def local_component(value: str) -> str:
    ...
    if len(extension.encode("utf-8")) > 32:
        extension = ""
    suffix = "~" + hashlib.sha256(encoded).hexdigest()[:20] + extension
    prefix = encoded[:MAX_COMPONENT_BYTES - len(suffix.encode("utf-8"))].decode("utf-8", errors="ignore")
...
def execute(parameters: dict, work_dir: Path, client: StorageGatewayClient) -> dict:
    ...
    storage_uri = client.publish(temporary, root_name, relative_path,
        f"download-remote-v2:{request_id}:{item_id}", size, content_sha256)
