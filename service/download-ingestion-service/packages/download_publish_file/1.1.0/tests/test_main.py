...
def test_detects_unnamed_jpeg_before_publication(tmp_path):
    data = b"\xff\xd8\xff\xe0" + b"image-data"
    source = tmp_path / "request" / "message-attachment-test.bin"
    source.parent.mkdir()
    source.write_bytes(data)
    output = {"requestId": "request", "itemId": "item", "fileName": source.name,
              "relativePath": "request/" + source.name, "sizeBytes": len(data),
              "contentSha256": hashlib.sha256(data).hexdigest()}
    context = {"parameters": {"resourceUsername": "owner", "assetMimeType": "application/octet-stream",
                              "receivedAt": "2026-09-09T00:00:00Z"},
               "stepOutputs": {"download_asset": output}}
    storage = Storage()
    result = MODULE.execute(context, tmp_path, storage)
    assert result["fileName"] == "message-attachment-test.jpg"
    assert result["mimeType"] == "image/jpeg"
    assert storage.arguments[2] == "owner/media/202609/20260909/message-attachment-test.jpg"
    assert source.read_bytes() == data
    assert output["fileName"].endswith(".bin")
