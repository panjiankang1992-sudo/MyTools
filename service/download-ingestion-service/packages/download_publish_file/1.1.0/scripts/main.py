...
def detected_media_type(source: Path) -> tuple[str, str] | None:
    """根据受限文件头识别图片格式，不依赖上游扩展名和 MIME。"""
...
def execute(context: dict, destination_root: Path, storage: StorageGatewayClient) -> dict:
    ...
    detected = detected_media_type(source)
    if detected is not None:
        # 校验后的内容类型决定目录及后续视觉标签，通用后缀替换为真实扩展名。
        parameters["assetMimeType"] = detected[0]
        if Path(str(output["fileName"])).suffix.lower() in {"", ".bin"}:
            output["fileName"] = str(Path(str(output["fileName"])).with_suffix(detected[1]))
