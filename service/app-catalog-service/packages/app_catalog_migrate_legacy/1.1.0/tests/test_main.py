...
def test_normalizes_timestamp_and_verifies_target_digest():
 connection=Connection();connection.apps[0]["created_time"]=datetime(2026,1,1,8);connection.apps[0]["update_time"]=datetime(2026,1,1,8)
 client=Client();dry=MODULE.execute(connection,client,"catalog-v2",True)
 assert client.values[0]["app"]["createdAt"]=="2026-01-01T00:00:00Z"
 client.result={"migrationKey":"catalog-v2","appCount":1,"versionCount":1,"fileCount":1,"collectionSha256":dry["digestSha256"]}
 applied=MODULE.execute(Connection(),client,"catalog-v2",False)
 assert applied["targetVerified"]is True
