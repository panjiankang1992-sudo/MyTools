...
def timestamp(value:object)->str:
    """将旧库无时区时间转换为 Java Instant 的 UTC 格式。"""
    if not isinstance(value,datetime):return str(value)
    current=(value if value.tzinfo is not None else value.replace(tzinfo=ZoneInfo("Asia/Shanghai"))).astimezone(timezone.utc)
...
def payload_digest(value:dict)->str:
    """按 Java 记录字段顺序计算单个聚合摘要。"""
    payload=[value["app"],value["versions"],value["files"]]
...
class Client:
    ...
    def evidence(self,migration_key:str)->dict:
        """读取指定迁移键的目标证据。"""
        request=Request(self.url+f"/internal/v1/catalog/migrations/legacy-apps/{migration_key}/evidence",headers={"Authorization":f"Bearer {self.token}","Accept":"application/json"})
        with self.opener(request,timeout=30)as response: body=response.read()
        if len(body)>1024*1024: raise RuntimeError("catalog evidence response is too large")
        value=json.loads(body.decode())
        if not isinstance(value,dict): raise RuntimeError("catalog evidence response is invalid")
        return value
...
def execute(connection,client:Client,migration_key:str,dry_run:bool)->dict:
    ...
    while True:
        ...
        for value in payloads:
            app_id=value["app"]["legacyId"];item_digest=payload_digest(value)
    ...
    if not dry_run:
        ...
        target_verified=evidence.get("migrationKey")==migration_key and evidence.get("appCount")==apps and evidence.get("versionCount")==versions and evidence.get("fileCount")==files and evidence.get("collectionSha256")==source_digest
        if not target_verified:raise RuntimeError("catalog target evidence does not match source")
    return {"migrationKey":migration_key,"dryRun":dry_run,"sourceHighWater":high_water,"apps":apps,"versions":versions,"files":files,"accepted":accepted,"skipped":skipped,"rejected":rejected,"digestSha256":source_digest,"targetVerified":target_verified}
