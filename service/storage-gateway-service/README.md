...
远端账户通过 `POST /api/internal/v1/storage/providers` 注册，只持久化 `secretRef`，响应不返回 remote 键或密钥引用。`GET /api/internal/v1/storage/providers/{id}/objects` 只允许调用服务端配置的回环 rclone RC `operations/list`，调用方不能提交 remote 名称或任意 RC 命令。递归扫描、复制和同步必须走异步任务；移动仍未开放。
