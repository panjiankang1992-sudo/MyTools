...
public class LegacyEbookProjectionService {
    ...
    public EbookPage list(long ownerId, String username, int page, int pageSize, String keyword) {
        if (ownerId <= 0 || username == null || !USERNAME.matcher(username).matches()
                || page < 1 || pageSize < 1 || pageSize > 100) throw new IllegalArgumentException("ebook query is invalid");
        ...
        try (var connection = DriverManager.getConnection(url, database.username(), database.password())) {
            ...
            // 新目录布局以可信认证用户名作为物理隔离边界。
            String root = "/opt/extend/resource/" + username + "/ebook";
