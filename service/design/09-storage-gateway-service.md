...
原生写入首批开放 `COPY_OBJECT`：来源支持 RCLONE 或 WebDAV，目标仅支持 WebDAV。操作创建阶段验证连接器能力并预占目标路径栅栏，任务参数只携带 UUID。Executor 通过操作专属端点有界下载来源、计算摘要、条件 PUT 目标、复读目标并逐字节摘要对账；Gateway 不提供可由脚本指定 Provider 或路径的通用写端点。WebDAV 使用 `If-None-Match: *` 阻止覆盖，V10 持久化本操作确认创建目标的所有权，补偿步骤只有在所有权成立时才删除；既有目标只复验不删除。成功终态在复验后的独立步骤提交，失败、超时和取消步骤补偿后再提交对应终态。S3 原生写入、树复制、移动和同步尚待实现。
