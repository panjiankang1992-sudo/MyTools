...
消息包含多个 URL 时使用 `download_message_url_batch`：父任务为每个 X URL 创建
`download_resolve_x_url` 只解析子任务，普通 HTTP URL 直接形成一个媒体项；收齐全部解析结果
后按稳定的消息动作序号和媒体序号排序。媒体总数超过阈值时，所有非大文件共享
`message-{messageId}` 目录；未超过阈值时都进入接收日期目录。大文件仍由发布原子步骤放入
`big_media` 独立资源包。随后每个媒体各自创建 `download_http_asset` 子任务并独立执行下载、
发布、资产登记和标签，父任务仅等待和聚合，不直接处理文件。
