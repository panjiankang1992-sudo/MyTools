...
当前迁移阶段使用 `assemble_executor_packages.py` 将各领域服务拥有的包源目录装配为 Executor
需要的 `{name}/{version}` 发布根。装配器先完整校验 manifest 身份和入口，拒绝重复版本、
绝对入口、目录穿越及任意符号链接；只复制运行文件，不复制测试和工具缓存，并生成包含每个
文件大小、SHA-256 及全发布摘要的 `package-index.json`。它只允许创建不存在的新目录，发布
切换由部署层更新 `TASK_EXECUTOR_SCRIPT_ROOT`，因此不会原地修改正在运行的节点目录。生产
签名可在该确定性索引之上增加，不改变任务定义引用。

运行时 `ScriptReleaseVerifier` 在索引存在时启动即流式复验全部文件，并在步骤执行前再次校验
入口，防止节点启动后入口被替换。索引中的包名、版本和相对路径仍按不可信输入处理，真实路径
必须留在脚本根内且不能经过符号链接。迁移期以
`TASK_EXECUTOR_REQUIRE_PACKAGE_INDEX=false` 兼容旧脚本目录；新发布节点完成装配后设为
`true`，索引缺失、重复身份、缺失入口或摘要变化都会阻止 Executor 启动。
...
- 全部 Scheduler 可用脚本包能够装配到新发布目录，发布索引与运行文件摘要一致。
