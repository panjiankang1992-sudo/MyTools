...
- Identity 首批登录和刷新入口已使用独立默认关闭开关、严格请求模型和稳定错误映射；开启入口时校验模式必须为 `DUAL` 或 `IDENTITY`，避免签发后无法使用的新令牌。
...
3. 客户端适配任务查询和取消。
4. Reader Facade、Drive 目录只读查询及 Identity 登录/刷新已替换为有界超时 HTTP 客户端，分别默认关闭；其余领域能力按相同模式逐项迁移。
