...
旧 ID 映射已由 `asset_migrate_legacy_mappings` 1.1.0 任务写入专用映射表，提供真实事务回滚式 dry-run、冻结源快照、游标、同迁移键重放、摘要和冲突报告；正式执行结束后必须从 Asset Registry 独立重算目标映射数量和集合摘要。旧物理路径只可作为经过 URI 安全校验的位置，不能成为新资产身份。独立源适配器已按真实 `local_file` schema 实现一致性捕获和原子封存，后续运行生产副本迁移。
...
- `asset_cutover_gate.py` 必须同时验证封存快照、dry-run、正式执行、同键重放和目标证据；源端拒绝数默认必须为零，已核实的无内容记录只能通过显式预期拒绝数保留为差异证据。
