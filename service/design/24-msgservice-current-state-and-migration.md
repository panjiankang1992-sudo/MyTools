...
当前已新增历史发件归档 schema、受保护批量迁移接口、幂等冲突检测、集合对账、适配器发件快照接口及一次性 SQLite 导出器。实时 IMAP 能力已在新 Messaging 中实现为默认关闭的 `message_poll_email` 任务：只读 UID 增量轮询、Message-ID 幂等、持久化检查点和附件流式物化均不改变旧服务。剩余执行工作是从远程运行库生成受控迁移产物、先 dry-run，再正式导入并完成逐项对账，以及在隔离账户副本上联调新 IMAP 任务；不引入消息总线、分布式事务或复杂切流机制。
