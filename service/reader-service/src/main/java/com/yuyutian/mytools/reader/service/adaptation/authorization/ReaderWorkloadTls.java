...
/** 加载 Reader 专属 PKCS12 凭据与限定信任库，用于向 Scheduler 发起 mTLS 内省请求。 */
@Component
public final class ReaderWorkloadTls {
    private final HttpClient client;
    private final String thumbprint;

    /** 建立受限信任库、证书身份和 HTTPS 主机名校验，不启用任何信任全部证书的降级。 */
    public ReaderWorkloadTls(ReaderWorkloadAuthorizationProperties properties) {
        ...
        try {
            URI scheduler = URI.create(properties.schedulerUrl());
            ...
            throw new IllegalStateException("Reader workload TLS initialization failed");
        } finally {
