...
已建立独立 `mytools_reader` schema 首版迁移，以及 `reader_source_search` 1.1.0 分片脚本。Scheduler 将一个任务实例展开为稳定的执行目标，脚本依据目标序号确定性分配书源，并使用执行实例隔离的 Reader Runtime 命名空间，避免并发分片互相覆盖书源快照。脚本保留部分成功、并发上限、跨书源书名去重和结果数量边界；现有 MyTools 搜索仍为线上权威实现。
