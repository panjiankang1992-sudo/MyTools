...
class WorkerTest(unittest.TestCase):
    ...
    def test_prompt_extraction_persists_and_replays_with_gpu_lease(self):
        ...
        with patch.dict(os.environ, {'IMAGE_PROMPT_VALIDATED': 'true', 'TAGGING_MODEL': 'vision-test'}), patch.object(worker, 'GpuLease') as lease, patch.object(worker, 'call', return_value={'done': True, 'message': {'content': json.dumps({'prompt': 'A watercolor forest'})}}) as call:
    ...
    def test_prompt_result_accepts_only_structured_final_description(self):
        for field in ['content', 'thinking']:
            self.assertEqual('A blue lake', worker.prompt_result({'done': True, 'done_reason': 'stop', 'message': {field: '{"prompt":"A blue lake"}'}}))
        invalid = [
            {'done': True, 'done_reason': 'length', 'message': {'content': '{"prompt":"Incomplete"}'}},
            {'done': True, 'message': {'thinking': 'I need to analyze the image carefully.'}},
            {'done': True, 'message': {'content': '{"prompt":"A lake","reasoning":"hidden"}'}},
            {'done': True, 'message': {'content': json.dumps({'prompt': 'x' * 4001})}},
            {'done': False, 'message': {'content': '{"prompt":"A lake"}'}}
        ]
        for reply in invalid:
            with self.assertRaises(ValueError): worker.prompt_result(reply)
