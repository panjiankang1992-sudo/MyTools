...
def prompt_result(reply):
    """只提取完整 JSON 中的提示词，不把模型思考或说明文字直接展示给用户。"""
    message = reply.get('message', {})
    if reply.get('done') is not True or reply.get('done_reason') == 'length' or not isinstance(message, dict):
        raise ValueError('IMAGE_RESPONSE_INVALID')
    # 当前受管模型可能将 JSON 放在 thinking 字段；两个字段均须通过相同结构校验。
    value = message.get('content') or message.get('thinking')
    if not isinstance(value, str) or len(value) > 32768:
        raise ValueError('IMAGE_RESPONSE_INVALID')
    try:
        result = json.loads(value)
    except (ValueError, TypeError):
        raise ValueError('IMAGE_RESPONSE_INVALID') from None
    if not isinstance(result, dict) or set(result) != {'prompt'}:
        raise ValueError('IMAGE_RESPONSE_INVALID')
...
def extract_prompt(parameters):
    ...
    instruction = ('Describe the visible image as a reusable image generation prompt. '
                   ...
                   'Return a JSON object with exactly one prompt string containing a concise paragraph of 100 to 150 words, within 4000 characters.')
    with GpuLease('tag'):
        reply = call(os.environ.get('TAGGING_SERVICE_URL', 'http://127.0.0.1:11434'), '/api/chat',
                     {'model': model, 'stream': False, 'think': False, 'format': 'json', 'keep_alive': 0,
                      ...
                      'options': {'temperature': 0.2, 'num_predict': 2048, 'num_ctx': 4096}}, timeout=600)
        prompt = prompt_result(reply)
