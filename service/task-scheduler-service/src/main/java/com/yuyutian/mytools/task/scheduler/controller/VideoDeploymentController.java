...
/** 运维身份通过审计接口控制固定视频任务，不允许任意修改调度定义。 */
@RestController
@RequestMapping("/internal/v1/video-generation-deployment")
public class VideoDeploymentController {
    private final JdbcTemplate jdbc;

    /** 注入调度数据库访问组件。 */
    public VideoDeploymentController(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public Map<String, Object> update(@Valid @RequestBody Deployment change, HttpServletRequest request) {
        ...
        jdbc.update("UPDATE execution_cluster SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND name=?",
                change.enabled(), "48615d43-0768-4d04-aa73-78fc97f57e47", "video-generation");
        jdbc.update("UPDATE task_definition SET enabled=?,version=version+1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND name=? AND enabled<>?",
                change.enabled(), "44544153-c55b-4124-8d7e-0365f70c3893", "video_generate", change.enabled());
        jdbc.update("INSERT INTO video_deployment_audit(id,enabled,service_id,created_at) VALUES(?,?,?,CURRENT_TIMESTAMP)",
