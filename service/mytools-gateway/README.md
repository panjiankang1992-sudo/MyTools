...
App Catalog 使用 `GATEWAY_APP_CATALOG_ROUTE_ENABLED` 开放已发布应用目录，只代理查询，不开放迁移或对账接口。DSH 使用 `GATEWAY_DSH_ROUTE_ENABLED` 开放当前主体的会话查询和归档；事件序号推进仍仅供 Connector 内部事件桥使用。两项开关默认均为关闭。
...
Drive 路由包括安全账户摘要列表、目录查询、索引刷新、单对象复制、递归目录树复制、统一操作查询和取消接口。Gateway 使用已验证用户 ID 构造内部 `ownerId`，不返回 remote key、外部账户标识、Secret 引用或底层任务标识，并以 `DRIVE_INTERNAL_TOKEN` 调用 Drive Service；移动和删除仍保留在旧入口。所有 Drive 路由继续受默认关闭的总开关和 owner 白名单保护。
