...
- [ ] 分租户或按任务类型灰度；Reader、Drive 与 Download 必须分别使用显式租户名单，空名单不得放量。
...
- [x] 建立独立 Gateway MVP、旧会话安全校验契约、默认关闭的 Identity 登录/刷新/当前会话注销入口、Reader 首批代理、Drive 目录只读代理和 Download 生命周期代理；Gateway 只转发重建后的载荷、只撤销实时校验绑定的会话并从已验证主体注入 owner，各路由关闭时不调用对应下游。
- [ ] 校验结果、性能、任务积压和错误率。
- [ ] 停止旧 Job，但保留快速回退。
- [ ] 观察期结束后删除旧执行路径。
