...
- [ ] 书源搜索、发现、电子书导入和索引。已建立独立 Reader Service、`mytools_reader` schema、原生多节点搜索分片、结果暂存与部分成功合并；Gateway 已接入 owner-bound 搜索、发现、健康检查与电子书导入生命周期。发现、健康检查、电子书导入、目录、章节预取和缓存清理均已任务化，书库派生索引可原子发布；现有电子书资产保持原位，不可再生书架、阅读进度和书签已具备幂等迁移，尚待使用实际数据导入。
